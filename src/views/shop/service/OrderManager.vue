<template>
  <div>
    <el-col :span="1">
      <el-tooltip
        :content="switchValue === '0' ? '按条件检索' : '关闭'"
        placement="bottom"
      >
        <el-switch v-model="switchValue" active-value="1" inactive-value="0">
        </el-switch>
      </el-tooltip>
    </el-col>
    ...
    <div v-show="switchValue === '1'">
      ...
      <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        <el-form-item label="最小年龄">
          <el-input
            v-model="searchForm.ageBegin"
            placeholder="请输入用户最小年龄"
          ></el-input>
        </el-form-item>
        <el-form-item label="最大年龄">
          <el-input
            v-model="searchForm.ageEnd"
            placeholder="请输入用户最大年龄"
          ></el-input>
        </el-form-item>
    ...
    <el-col :span="14" :offset="5">
      <el-tabs v-model="activeName">
            ...
            <el-table-column type="expand" border stripe highlight-current-row>
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
                  <!--       FIXME: 多个食物，多张卡片         -->
                  <el-card class="box-card" v-for="card in props.row" :key="card">
                      ...
                      <el-aside>
                        ...
                        <el-form-item>
                          <div>
                            ...
                            <el-image
                              v-if="
                                card.picture === '' || card.picture === null
                              "
        ...
        <el-tab-pane label="未完成" name="未完成">
          <el-table
            :data="dataInPage.filter(x => x.some(y => y.state === '未完成'))"
          >
            <el-table-column type="expand" border stripe highlight-current-row>
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
        ...
        <el-tab-pane label="已完成" name="已完成">
          <el-table
            :data="dataInPage.filter(x => !x.some(y => y.state !== '已完成'))"
          >
            <el-table-column type="expand" border stripe highlight-current-row>
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
        ...
        <el-tab-pane label="已取消" name="已取消">
          <el-table
            :data="dataInPage.filter(x => !x.some(y => y.state !== '已取消'))"
          >
            <el-table-column type="expand" border stripe highlight-current-row>
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
...
export default {
  ...
  created() {
    this.$get("/shop/getOrderHistory", {
      ...
      .then(res => {
        ...
        this.formatData = util.filterByTimeAndName(
          this.orderList,
          "time",
          "consumerName"
        );
  ...
  methods: {
      ...
      this.$confirm("将确认接收所有未完成的订单,是否继续?", {
        ...
        .then(async () => {
          for (let order of row) {
            ...
            await this.$get("/shop/confirmOrder", { OID: order.id }).then(
              res => {
                for (let i = 0; i < this.orderList.length; i++) {
    ...
    searchConsumers() {
      ...
      this.searchConsumerRes = this.allConsumers
        .filter(
          x =>
            !this.searchForm.id || Number(this.searchForm.id) === Number(x.cId)
        )
        .filter(
          x =>
            !this.searchForm.name ||
            x.cName.toLowerCase().includes(this.searchForm.name.toLowerCase())
        )
        .filter(x => !this.searchForm.sex || this.searchForm.sex === x.cSex)
        .filter(
          x =>
            !this.searchForm.ageBegin ||
            Number(this.searchForm.ageBegin) <= x.cAge
        )
        .filter(
          x =>
            !this.searchForm.ageEnd || Number(this.searchForm.ageEnd) >= x.cAge
        )
        .filter(
          x => !this.searchForm.tel || x.cTel.includes(this.searchForm.tel)
        )
        .filter(
          x =>
            ...
            x.cAddress
              .toLowerCase()
              .includes(this.searchForm.address.toLowerCase())
        )
        .map(x => x.cId);
  ...
  computed: {
    ...
    dataInPage() {
      return this.formatData.filter(x =>
        this.searchConsumerRes.includes(x.cid)
      );
    }
  }
};
