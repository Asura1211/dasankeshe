  ...
  <div>
          ...
          <el-col :offset="1">
            <el-table :data="dataInPage">
              ...
              >
                <template slot-scope="scope">
                  <el-link
                    @click.native="$router.push(`/shop/${scope.row.sid}`)"
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitComment(nowSubmit)"
          >确 定</el-button
        >
      </div>
    </el-dialog>
      ...
      this.$confirm("注意:此操作只会取消此订单内未完成的商品,是否继续?", {
        ...
        .then(async () => {
          for (let order of row) {
            ...
            await this.$get("/consumer/cancelOrder", { OID: order.id }).then(
              res => {
                for (let i = 0; i < this.orderList.length; i++) {
